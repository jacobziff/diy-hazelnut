...
let rec syn = (ctx: typctx, e: Hexp.t): option(Htyp.t) => {
  switch e {
    ...
    | Ap(e1, e2) => { // SAp
      ...
      if (ana(ctx, e2, t2)) Some(t') else None
    }
    | Lit(_) => Some(Num) // SNum
    | Plus(e1, e2) => { // SPlus
      if (ana(ctx, e1, Num) && ana(ctx, e2, Num)) Some(Num)
      else None
    }
...
let rec typ_action = (t: Ztyp.t, a: Action.t): option(Ztyp.t) => {
  switch (t, a) {
    | (Cursor(Arrow(t1, t2)), Move(Child(One))) => // TMArrChild1
      Some(Ztyp.LArrow(Cursor(t1), t2))
    | (Cursor(Arrow(t1, t2)), Move(Child(Two))) => // TMArrChild2
      Some(Ztyp.RArrow(t1, Cursor(t2)))
    | (LArrow(Cursor(t1), t2), Move(Parent)) => // TMArrParent1
      Some(Cursor(Arrow(t1, t2)))
    | (RArrow(t1, Cursor(t2)), Move(Parent)) => // TMArrParent2
      Some(Cursor(Arrow(t1, t2)))
    | (Cursor(_), Del) => Some(Cursor(Hole)) // TMDel
    | (Cursor(t), Construct(Arrow)) => // TMConsArr
      Some(RArrow(t, Cursor(Hole)))
    | (Cursor(Hole), Construct(Num)) => // TMConsNum
      Some(Cursor(Num))
    | (LArrow(t', t), _) => // TMArrZip1
      let* t' = typ_action(t', a);
      Some(Ztyp.LArrow(t', t))
    | (RArrow(t, t'), _) => // TMArrZip2
      let* t' = typ_action(t', a);
...
and exp_action = (e: Zexp.t, d: Dir.t): option(Zexp.t) => {
  switch (e, d) {
    | (Cursor(Asc(e, t)), Child(One)) => // EMAscChild1
      Some(LAsc(Cursor(e), t))
    | (Cursor(Asc(e, t)), Child(Two)) => // EMAscChild2
      Some(RAsc(e, Cursor(t)))
    | (LAsc(Cursor(e), t), Parent) => // EMAscParent1
      Some(Cursor(Asc(e, t)))
    | (RAsc(e, Cursor(t)), Parent) => // EMAscParent2
      Some(Cursor(Asc(e, t)))
    | (Cursor(Lam(x, e)), Child(One)) => // EMLamChild1
      Some(Lam(x, Cursor(e)))
    | (Lam(x, Cursor(e)), Parent) => // EMLamParent
      Some(Cursor(Lam(x, e)))
    | (Cursor(Plus(e1, e2)), Child(One)) => // EMPlusChild1
      Some(LPlus(Cursor(e1), e2))
    | (Cursor(Plus(e1, e2)), Child(Two)) => // EMPlusChild2
      Some(RPlus(e1, Cursor(e2)))
    | (LPlus(Cursor(e1), e2), Parent) => // EMPlusParent1
      Some(Cursor(Plus(e1, e2)))
    | (RPlus(e1, Cursor(e2)), Parent) => // EMPlusParent2
      Some(Cursor(Plus(e1, e2)))
    | (Cursor(Ap(e1, e2)), Child(One)) => // EMApChild1
      Some(LAp(Cursor(e1), e2))
    | (Cursor(Ap(e1, e2)), Child(Two)) => // EMApChild2
      Some(RAp(e1, Cursor(e2)))
    | (LAp(Cursor(e1), e2), Parent) => // EMApParent1
      Some(Cursor(Ap(e1, e2)))
    | (RAp(e1, Cursor(e2)), Parent) => // EMApParent2
      Some(Cursor(Ap(e1, e2)))
    | (Cursor(NEHole(e)), Child(One)) => // EMNEHoleChild1
      Some(NEHole(Cursor(e)))
    | (NEHole(Cursor(e)), Parent) => // EMNEHoleParent
...
and ana_action =
  ...
  switch (e, t, a) {
    | (e', _, Move(d)) => // AAMove
      exp_action(e', d)
    | (Cursor(_), _, Del) => // AADel
      Some(Cursor(EHole))
    | (Cursor(e), _, Construct(Asc)) => // AAConAsc
      Some(RAsc(e, Cursor(t)))
    | (Cursor(EHole), _, Construct(Var(x))) => // AAConVar
      let t' = TypCtx.find(x, ctx);
      if (inconsistent(t, t')) 
        Some(NEHole(Cursor(Var(x))))
      else
        None // FIXME: This could be a bug
    ...
    | (Cursor(EHole), _, Construct(Lit(n))) => // AAConNumLit
      if (inconsistent(t, Num))
        Some(NEHole(Cursor(Lit(n))))
      else
        None // FIXME: This could be a bug
    | (Cursor(e), _, Finish) => // AAFinish
      if (ana(ctx, e, t))
        Some(Cursor(e))
      else
        None
    | (Lam(x, e'), _, _) => // AAZipLam
